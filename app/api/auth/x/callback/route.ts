 ...
 *
 * X (Twitter) OAuth 2.0 callback. Exchanges the authorization code for tokens
 * and creates a SocialConnection for the workspace.
 */
...
export async function GET(req: Request) {
  ...
  if (error) {
    return redirect(`/settings/social?error=x_rejected&detail=${encodeURIComponent(error)}`)
  }
  ...
  // Extract X user id from raw response
  let externalAccountId = `x_${Date.now()}`
  ...
  try {
    ...
    if (raw?.["user_id"]) {
      ...
      externalAccountId = String(d.id ?? externalAccountId)
      displayName = String(d.name ?? d.username ?? displayName)
    }
  } catch {
    // use fallback
  }
  ...
  try {
    ...
    console.error("[x-callback] connection save failed", err)
    return redirect(`/settings/social?error=save_failed`)
  }

  return redirect(`/settings/social?connected=x`)
}
