...
// ─────────────────────────────────────────────────────────────────────────────
// INBOX ROUTE DISABLED — commented out while the WhatsApp integration is parked.
// The route now 404s. To restore: delete this stub and uncomment the original
// implementation below (and re-enable the Inbox nav item in components/shell/sidebar.tsx).
// ─────────────────────────────────────────────────────────────────────────────
...
//   const { c: selectedId, channel: rawChannel } = await searchParams
//
//   // Integration parked: drop the WhatsApp channel filter and outbound composer,
//   // and treat a lingering ?channel=WHATSAPP link as "all".
//   const waOn = whatsappEnabled()
//   const channels = waOn ? CHANNELS : CHANNELS.filter((ch) => ch.id !== "WHATSAPP")
//   const filterChannel = !waOn && rawChannel === "WHATSAPP" ? "all" : rawChannel
//
...
//                 : "Messages from your contacts appear here automatically."}
//             </p>
//           </div>
//         ) : (
//           contacts.map((c) => (
